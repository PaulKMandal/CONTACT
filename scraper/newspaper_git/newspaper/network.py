...
def get_html(url, config=None, response=None):
    ...
    try:
        test = get_html_2XX_only(url, config, response)
        #print(test)
...
def _get_html_from_response(response, config):
    if response.headers.get('content-type') in config.ignored_content_types_defaults:
        #print("_get_html_from_response first if")
        return config.ignored_content_types_defaults[response.headers.get('content-type')]
    if response.encoding != FAIL_ENCODING:
        # return response as a unicode string
        #print("_get_html_from_response second if")
        #print("response",response)
        #print("repsonse.text",response.text)
        #print("branch html",html)
        html = response.text
    else:
        #print("_get_html_from_response first else")
        html = response.content
        if 'charset' not in response.headers.get('content-type'):
            #print("_get_html_from_response third if")
            encodings = requests.utils.get_encodings_from_content(response.text)
            if len(encodings) > 0:
                #print("_get_html_from_response fourth if")
